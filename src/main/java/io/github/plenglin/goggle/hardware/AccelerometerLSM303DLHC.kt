...
class AccelerometerLSM303DLHC(val dev: I2CDevice, val scale: AccelerometerLSM303DLHCScale) : Accelerometer, Command() {
    ...
    override fun update(dt: Int) {
        acceleration = Vector3D(
                multiplier * ((dev.read(0x28) shl 8) or dev.read(0x29)).toDouble(),
                multiplier * ((dev.read(0x2a) shl 8) or dev.read(0x2b)).toDouble(),
                multiplier * ((dev.read(0x2c) shl 8) or dev.read(0x2d)).toDouble()
        )
    }
